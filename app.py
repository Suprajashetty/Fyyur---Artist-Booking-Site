...
def search_venues():
  ...
  searchTxt = request.form['search_term']
  data = []
  requiredVenues = Venue.query.filter(Venue.name.ilike('%'+searchTxt+'%'))
...
def search_artists():
  ...
  searchTxt = request.form['search_term']
  data = []
  requiredVenues = Artist.query.filter(Artist.name.ilike('%'+searchTxt+'%'))
